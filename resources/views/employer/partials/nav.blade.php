...
<aside class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
    <div class="flex items-center gap-3 border-b border-slate-100 pb-4">
        @if ($employer->logo_path)
            <img src="{{ asset($employer->logo_path) }}" alt="{{ $employer->name }}" class="h-12 w-12 rounded border border-slate-200 object-contain">
        @else
            <div class="flex h-12 w-12 items-center justify-center rounded bg-[#2a7190] font-bold text-white">{{ str($employer->name)->substr(0, 1) }}</div>
        @endif
        <div class="min-w-0">
            <p class="truncate font-bold">{{ $employer->name }}</p>
            <p class="text-xs font-semibold uppercase text-[#2a7190]">{{ str($employer->billing_plan)->headline() }} employer</p>
        </div>
    </div>
