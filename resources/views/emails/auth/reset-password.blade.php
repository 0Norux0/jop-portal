@php
    ...
    $content = $content ?? \App\Support\EmailContent::load()['reset_password'];
@endphp
<!doctype html>
<html lang="en">
<head>
                    ...
                    <tr>
                        <td style="padding:30px 28px;">
                            ...
                            <p style="margin:26px 0;">
                                <a href="{{ $resetUrl }}" style="display:inline-block;background:{{ $primaryColor }};color:#ffffff;text-decoration:none;font-weight:700;padding:13px 20px;border-radius:6px;">{{ $content['button_label'] }}</a>
                            </p>
                            <p style="margin:0 0 14px;font-size:14px;line-height:1.6;color:#526173;">{{ $content['note'] }}</p>
                            <p style="margin:22px 0 0;font-size:13px;line-height:1.6;color:#718096;">If the button does not work, copy this link into your browser:<br><a href="{{ $resetUrl }}" style="color:{{ $primaryColor }};word-break:break-all;">{{ $resetUrl }}</a></p>
                        </td>
                    </tr>
                    <tr>
